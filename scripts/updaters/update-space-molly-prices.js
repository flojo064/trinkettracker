// Update Space Molly prices from Mercari scraper results to scraped-prices.json
const fs = require('fs');
const path = require('path');

const spaceMollyMercariPath = path.join(__dirname, '..', '..', 'data', 'space-molly-mercari-prices.json');
const scrapedPricesPath = path.join(__dirname, '..', '..', 'data', 'scraped-prices.json');

// Load the Mercari prices
const spaceMollyMercariPrices = JSON.parse(fs.readFileSync(spaceMollyMercariPath, 'utf8'));
...
Object.entries(spaceMollyMercariPrices).forEach(([seriesId, seriesData]) => {
...
console.log(`✅ Updated ${updatedCount} MEGA SPACE MOLLY items in scraped-prices.json`);
console.log(`📊 MEGA SPACE MOLLY price summary:`);
