// Mercari Price Scraper for Oipippi
// Scrapes current Mercari listings for Oipippi figures
...
// Load Oipippi brand data
const loadOipippiData = () => {
  const oipippiPath = path.join(__dirname, "data", "oipippi.json");
  const data = fs.readFileSync(oipippiPath, "utf8");
  return JSON.parse(data);
};

// Save Mercari price data
const saveMercariPrices = (prices) => {
  const outputPath = path.join(__dirname, "data", "oipippi-mercari-prices.json");
...
const generateSearchTerm = (seriesName, itemName) => {
  ...
  // Search: "Oipippi" + series name + item name
  return `Oipippi ${cleanSeries} ${cleanItem}`;
};
...
const normalizeForMatching = (text) => {
  return text.toLowerCase()
    .replace(/\b(the|a|an|oipippi|series)\b/g, '')
    .replace(/\s+/g, ' ')
    .trim();
};
...
const filterRelevantItems = (listings, itemName, seriesName) => {
  return listings.filter(listing => {
    ...
    // Must contain "oipippi" (check various spellings)
    if (!title.includes('oipippi') && !title.includes('oipipi') && !title.includes('oi pippi')) {
      return false;
    }
    ...
    // For named series, verify series name appears if it's specific
    ...
    if (seriesWords.length > 0) {
      ...
      // If we have a specific series name and it's not in the title, use fuzzy matching
      if (!hasSeriesName) {
        const normalizedItem = normalizeForMatching(itemNameLower);
        const normalizedTitle = normalizeForMatching(title);
        const similarity = calculateSimilarity(normalizedItem, normalizedTitle);
        if (similarity < 0.6) {
          return false;
        }
      }
    ...
    // Fuzzy item matching with normalization
    const normalizedItem = normalizeForMatching(itemNameLower);
    const normalizedTitle = normalizeForMatching(title);
    ...
    // Fallback: check if significant words appear
    const itemWords = normalizedItem.split(/\s+/).filter(w => w.length > 2);
    if (itemWords.length === 0) return true;
    
    // Need at least some keywords to match
    const matchCount = itemWords.filter(word => normalizedTitle.includes(word)).length;
    return matchCount >= Math.min(2, itemWords.length);
  });
};

// Scrape prices for all Oipippi items
const scrapeMercariPrices = async () => {
  console.log("🚀 Starting Mercari price scraping for Oipippi...\n");
  ...
  const oipippiData = loadOipippiData();
  const mercariPrices = {};

  let totalItems = 0;
  let itemsWithPrices = 0;

  for (const series of oipippiData.series || []) {
    ...
    for (const item of series.items || []) {
      ...
      // Strategy 1: Full search with series
      const fullSearch = generateSearchTerm(series.name, item.name);
      ...
      // Strategy 2: If not enough, try just brand + item name
      if (allRelevantItems.length < 5) {
        const simpleSearch = `Oipippi ${item.name}`;
