// Mercari Price Scraper for MEGA SPACE MOLLY
// Scrapes current Mercari listings for Space Molly figures
...
// Series to skip
const SKIP_SERIES = ['space-molly-limited-editions'];
...
// Load Space Molly brand data
const loadSpaceMollyData = () => {
  const spaceMollyPath = path.join(__dirname, "data", "space-molly.json");
  const data = fs.readFileSync(spaceMollyPath, "utf8");
  return JSON.parse(data);
};

// Save Mercari price data
const saveMercariPrices = (prices) => {
  const outputPath = path.join(__dirname, "data", "space-molly-mercari-prices.json");
...
const generateSearchTerm = (seriesName, itemName) => {
  ...
  // Search: "Space Molly" + series name + item name
  return `Space Molly ${cleanSeries} ${cleanItem}`;
};

// Remove articles and common words for better matching
const normalizeForMatching = (text) => {
  return text.toLowerCase()
    .replace(/\b(the|a|an|space|molly|mega)\b/g, '')
    .replace(/\s+/g, ' ')
    .trim();
};
...
const filterRelevantItems = (listings, itemName, seriesName) => {
  return listings.filter(listing => {
    ...
    // Must contain "molly" or "space molly"
    if (!title.includes('molly')) return false;
    ...
    // Handle series matching - check if series keywords are present
    const hasSeriesMatch = seriesName.toLowerCase().split(/\s+/)
      .filter(w => w.length > 3 && !['series', 'molly', 'space', 'mega'].includes(w))
...
// Scrape prices for all Space Molly items
const scrapeMercariPrices = async () => {
  console.log("🚀 Starting Mercari price scraping for MEGA SPACE MOLLY...\n");
  ...
  const spaceMollyData = loadSpaceMollyData();
  const mercariPrices = {};
  ...
  for (const series of spaceMollyData.series || []) {
    // Skip Limited Editions series
    if (SKIP_SERIES.includes(series.id)) {
      console.log(`\n⏭️  Skipping series: ${series.name} (${series.items.length} items)`);
      skippedSeries += series.items.length;
      continue;
    }
    ...
    for (const item of series.items || []) {
      ...
      if (allRelevantItems.length < 5) {
        const simpleSearch = `Space Molly ${item.name}${secretSuffix}`;
  ...
  console.log(`   • Items skipped: ${skippedSeries}`);
