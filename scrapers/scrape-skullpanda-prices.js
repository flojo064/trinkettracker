// Mercari Price Scraper for SkullPanda
// Scrapes current Mercari listings for SkullPanda figures
...
// Load SkullPanda brand data
const loadSkullPandaData = () => {
  const skullpandaPath = path.join(__dirname, "data", "skullpanda.json");
  const data = fs.readFileSync(skullpandaPath, "utf8");
  return JSON.parse(data);
};

// Save Mercari price data
const saveMercariPrices = (prices) => {
  const outputPath = path.join(__dirname, "data", "skullpanda-mercari-prices.json");
...
const generateSearchTerm = (seriesName, itemName) => {
  ...
  // Search: "SkullPanda" + series name + item name
  return `SkullPanda ${cleanSeries} ${cleanItem}`;
};

// Remove articles and common words for better matching
const normalizeForMatching = (text) => {
  return text.toLowerCase()
    .replace(/\b(the|a|an|skullpanda|skull|panda)\b/g, '')
    .replace(/\s+/g, ' ')
    .trim();
};
...
const filterRelevantItems = (listings, itemName, seriesName) => {
  return listings.filter(listing => {
    ...
    // Must contain "skullpanda" or "skull panda"
    if (!title.includes('skullpanda') && !title.includes('skull panda')) return false;
    ...
    if (seriesWords.length > 0) {
      ...
      // If we have a specific series name and it's not in the title, be lenient
      // (SkullPanda series names are sometimes not mentioned)
    }
...
// Scrape prices for all SkullPanda items
const scrapeMercariPrices = async () => {
  console.log("🚀 Starting Mercari price scraping for SkullPanda...\n");
  ...
  const skullpandaData = loadSkullPandaData();
  const mercariPrices = {};

  let totalItems = 0;
  let itemsWithPrices = 0;

  for (const series of skullpandaData.series || []) {
    ...
    for (const item of series.items || []) {
      ...
      if (allRelevantItems.length < 5) {
        const simpleSearch = `SkullPanda ${item.name}${secretSuffix}`;
